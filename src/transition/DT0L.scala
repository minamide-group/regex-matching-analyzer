...
class DT0L[A,Q](
  ...
  def toLabeledGraph(): LabeledGraph[Q,A] = {
    ...
    new LabeledGraph(labeledEdges)
  }

  def calcGrowthRate(initials: Set[Q]): (Option[Int], Witness[A]) = {
    val graph = toLabeledGraph().reachablePartFrom(initials)
    ...
    def calcGrowthRate(checkIDA: (Set[Q],Set[Q]) => Option[Pump]): (Option[Int], Seq[Pump]) = {
      def checkEDA(): Option[Pump] = {
        ...
        scGraph.nodes.toStream.map(checkEDA).find(_.isDefined).map(_.get)
      }
    ...
    val (ambiguity, pumps) = calcGrowthRate(checkIDA)
    (ambiguity, generateWitness(pumps))
  }

  def calcGrowthRate(initial: Q): (Option[Int], Witness[A]) = calcGrowthRate(Set(initial))
...
class IndexedDT0L[A,B,P](
  indexedMorphs: Map[(P,P), Map[A, Map[B,Seq[B]]]]
) {
  def toDT0L(): DT0L[(A,P),(B,P)] = {
